...
public class Test01Static {
    ...
    /*
     1、static
      ...
      a、成员变量
        被 static 修饰的成员变量称为：类变量、静态变量、静态字段，其特点：
          ⚠️ 在程序运行过程中只占用一份固定的内存（‌JDK1.2~JDK6存储在方法区；JDK7+ 存储在堆内存的静态存储区（静态域）中）；
          当类首次被加载时，JVM会在堆中为类变量分配内存空间；
          可以通过任意实例、类访问；
     ...
     2、使用 static 注意点
      不推荐使用实例访问类变量、类方法；
      在同一个类中，不能有同名的 实例变量 和 类变量，不能有相同签名的实例方法和类方法。
