...
// 导入其它包
import com.czm.api_package.sub.Car;
...
public class Test {
    /*
     1、包 package
       ...
       b、包的命名建议
            为保证包名的唯一性，一般 包名称 都是以 公司域名 的倒写开头，比如：com.baidu.*。
            包名称 需全小写（以避免与某些类名或者接口名冲突）。

       c、类的第一句代码必须使用 package 声明自己属于那个包，否则会报错 ⚠️
            比如：
                  package com.czm.model;
                  public class Dog {

                  }
          如果类声明在 src 目录下，则不需要声明属于那个包。

     2、包名称细节
        如果公司域名有非法字符（如：包名称不能使用数字），建议添加下划线(_)来使包名合法化。如下所示：
                域名                  包名
    ...
    public static void main(String[] args) {
        ...
        com.czm.api_package.sub.Student s2 = new com.czm.api_package.sub.Student();
        s2.say();
    }
}
